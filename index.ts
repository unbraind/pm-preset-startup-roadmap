    ...
    api.registerCommand({
      ...
      async run(context: CommandHandlerContext) {
        ...
        if (!fs.existsSync(pmDir)) {
          console.error(
            `Error: pm workspace not found at ${pmDir}\n` +
        ...
        if (settingsExist && !force) {
          console.warn(
            `Warning: ${settingsPath} already exists — skipping.\n` +
        ...
        writeJsonFile(
          path.join(templatesDir, "initiative.json"),
          TEMPLATE_INITIATIVE,
          dryRun,
          "initiative.json"
        );
        ...
        writeJsonFile(
          path.join(templatesDir, "milestone.json"),
          TEMPLATE_MILESTONE,
          dryRun,
          "milestone.json"
        );
